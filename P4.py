...
# ---
# 
# * Estudiante: **Jorge Moisés Sandí Delgado**
# * Carné: **B77173**
# * Grupo: **2**
# 
# ---
...
def modulador(bits, fc, mpp):
    ...
    for b1, b2, b3, b4 in bits2:
        if b1 == 0 and b2 == 0:
            senal_Tx[i*mpp : (i+2)*mpp] = portadoraQ * -3

        if b1 == 0 and b2 == 1:
            senal_Tx[i*mpp : (i+2)*mpp] = portadoraQ * -1
        ...
        if b3 == 0 and b4 == 0:
            senal_Tx[(i+2)*mpp : (i+4)*mpp] = portadoraI * 3
 
        if b3 == 0 and b4 == 1:
            senal_Tx[(i+2)*mpp : (i+4)*mpp] = portadoraI* 1
    ...
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    print(portadora)
    return senal_Tx, P_senal_Tx, portadoraI, portadoraQ, moduladora
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadoraI, portadoraQ, moduladora = modulador(bits_Tx, fc, mpp)
